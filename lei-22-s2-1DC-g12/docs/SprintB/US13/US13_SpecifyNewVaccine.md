...
| Step 2 		 |							 |             |                              |
| Step 3         |  ...getting the vaccine type? | VaccineType |IE: knows its own data (e.g. code). |
| Step 4  		 |							 |             |                              |
| Step 5  		 |	...instantiating a new vaccine? | RegisterVaccine  | Creator (Rule 1): RegisterVaccine contains class Vaccine objects. |
...
| Step 8  		 |							 |             |                              |              
| Step 9  		 | 	...saving the inputted data? | RegisterVaccine | HC + LC: object created in step 1 has its own data. |
|	             |  ... validating all data (local validation)? | Vaccine | IE: owns its data.| 
| 			  	 |	... validating all data (global validation)? | RegisterVaccine | HC + LC: knows all its vaccine.| 
| 			  	 |	... saving the created vaccine? | RegisterVaccine | HC + LC: owns all its vaccines.| 
